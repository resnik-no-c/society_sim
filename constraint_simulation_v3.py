...
class SimulationConfig:
    ...
    def __post_init__(self):
        ...
        assert 0.95 <= self.out_group_trust_bias <= 1.05, f"Invalid out_group_trust_bias: {self.out_group_trust_bias}"
        ...
        assert 1.4 <= self.group_trust_bias <= 1.6, f"Invalid group_trust_bias: {self.group_trust_bias}"
...
class OptimizedPerson:
    ...
    def update(self, system_stress: float, params: SimulationConfig, cooperation_bonus: float = 0):
        ...
        pressure_decay = self.stress_recovery_rate * need_satisfaction * 1.35 # ↑ accelerate recovery so empirical half-life ≈ 8–10 yrs
...
def sample_config() -> SimulationConfig:
    ...
    try:
        ...
        config = SimulationConfig(
            ...
            out_group_trust_bias=random.uniform(0.95, 1.05),
            ...
            group_trust_bias=random.uniform(1.4, 1.6),
            resilience_profile={
...
class EnhancedMassSimulation:
    ...
    def __init__(self, params: SimulationConfig, run_id: int):
        ...
        sample_size = max(1, int(len(self.people) * self.params.critical_sample_rate))
        self.critical_agent_ids = {p.id for p in random.sample(self.people, sample_size)}

        # Store initial state for results generation
        self.initial_trait_avg = self._get_average_traits()
        self.initial_group_populations = self._get_group_populations()
    ...
    def _get_average_traits(self) -> Dict[str, float]:
        ...
        alive_people = [p for p in self.people if not p.is_dead]  # Include all living people
        if not alive_people:
    ...
    def _generate_emergency_result(self) -> EnhancedSimulationResults:
        ...
        try:
            ...
            try:
                final_traits = self._get_average_traits()
                initial_traits = final_traits.copy()  # Use final as approximation for initial in emergency
            except:
                final_traits = {'physiological': 0, 'safety': 0, 'love': 0, 'esteem': 0, 'self_actualization': 0}
                initial_traits = final_traits.copy()
        ...
        for _ in range(target_rounds):
            ...
            if sim.round % 4 == 0:
                ...
                row_data = {
                    ...
                    'current_cooperation_rate':   S(sim, 'total_mutual_cooperation') / max(1, S(sim, 'total_interactions')),
                    'current_population':         len([p for p in sim.people if not p.is_dead]),
                    'current_system_stress':      S(sim, 'system_stress'),

                    # — system dynamics so far —
                    'first_cascade_round':        S(sim, 'first_cascade_round'),
                    'total_cascade_events':       S(sim, 'total_cascade_events'),
                    'total_shock_events':         S(sim, 'total_shock_events'),
                    'total_defections':           S(sim, 'total_defections'),
                    'total_redemptions':          S(sim, 'total_redemptions'),
                    'redemption_rate':            S(sim, 'total_redemptions') / max(1, S(sim, 'total_defections') + S(sim, 'total_redemptions')),
                    'total_interactions':         S(sim, 'total_interactions'),
                    'total_mutual_cooperation':   S(sim, 'total_mutual_cooperation'),
                    'total_mutual_defection':     S(sim, 'total_mutual_defection'),
                    'total_mixed_outcomes':       S(sim, 'total_mixed_outcomes'),
                    ...
                    'social_cohesion_factor':     sim.params.social_diffusion * S(sim, 'avg_trust_level'),
                }
